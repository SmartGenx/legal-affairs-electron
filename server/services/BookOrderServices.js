...
class BookOrderService {
  async getAllBookOrders(BookOrderFilter) {
    try {
      ...
      if (BookOrderFilter.id) {
        BookOrderFilter.id = parseInt(BookOrderFilter.id, 10)
        if (isNaN(BookOrderFilter.id)) {
          throw new Error('Invalid ID: ID must be a number.')
        }
      }
      if (BookOrderFilter.orderNumber) {
        BookOrderFilter.orderNumber = parseInt(BookOrderFilter.orderNumber, 10)
        console.log(
          ':rocket: ~ BookOrderService ~ getAllBookOrders ~ BookOrderFilter.orderNumber:',
          typeof BookOrderFilter.orderNumber
        )
      }
      if (BookOrderFilter.id) {
        BookOrderFilter.id = parseInt(BookOrderFilter.id, 10)
