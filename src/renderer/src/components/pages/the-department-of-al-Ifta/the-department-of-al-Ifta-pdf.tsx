    ...
    <div ref={ref} className="w-[95%] mx-auto h-full relative">
      ...
      <table dir="rtl" className="w-full h-full rounded-t-lg">
        <tr className="  h-10 text-black border-2 border-gray-300 py-5 bg-[#eef0f8]">
          ...
          <th className="translate-x-2 font-arial">مقدم الرأي</th>
          <th className="translate-x-4 font-arial">مصدر التوجية</th>
          <th className="translate-x-2 font-arial">موضوع الرأي</th>
          <th className="font-arial">رقم الرأي</th>
          <th className="translate-x-7 font-arial">تاريخ الرأي والإفتاء</th>
        </tr>
        {data.map((item, index) => (
          ...
          <tr className=" -translate-x-3 h-10 text-black border-2 border-gray-300 py-5">
            ...
            <td className="font-arial">{new Date(item.date).toISOString().split('T')[0]}</td>
          </tr>
        ))}
      </table>
      {/* Footer for Print */}
