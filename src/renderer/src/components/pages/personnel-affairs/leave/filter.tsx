...
const FilterDrawer = () => {
  ...
  const [_selectedGender, setSelectedGender] = useState<string>('')

  const [reference, setReference] = useState('')
  console.log('🚀 ~ FilterDrawer ~ reference:', reference)

  useEffect(() => {
    const leaveTypeId = searchParams.getAll('leaveTypeId')
  ...
  const handleFilter = () => {
    ...
    selectedLeaveTypeId.forEach((id) => params.append('leaveTypeId', id))
    selectedLevel.forEach((id) => params.append('IssueDetails[some][level]', id))

    if (dateFrom) {
      params.set('startDate[gte]', dateFrom?.toISOString())
    }

    if (dateTo) {
      params.set('startDate[lte]', dateTo?.toISOString())
    }
    if (reference) {
              ...
              {leaveType?.data?.map((governorate) => (
                <div key={governorate.id} className="flex items-center space-x-2">
                  <input
                    ...
                    checked={selectedLeaveTypeId.includes(String(governorate.id))}
                    onChange={() => handleGovernorateChange(String(governorate.id))}
