...
const LeaveSearch = () => {
  ...
  const { data: accredited } = useQuery({
    ...
    queryFn: () =>
      getApi<Accrediteds[]>('/leave-details', {
        params: {
          'include[employ]': true
        },
        headers: {
  ...
  const loadOptions = async (value: string) => {
    if (!value) return []
    const data = await getApi<Accrediteds[]>('/leave-details', {
      params: {
        'include[employ]': true,
        'employ[name][contains]': value
      },
      headers: {
        Authorization: authToken()
      }
  ...
  const onChange = (val: Accrediteds | null) => {
    ...
    if (val?.employ.name) {
      params.set('query', val.employ.name)
    } else {
      params.delete('query')
    }
    params.set('page', '1')
      ...
      <div className="!my-10 flex flex-col items-center justify-between sm:flex-row">
        <div className="flex w-full items-center justify-center rounded-xl border-[3px] border-[#E5E7EB] ">
          ...
          <AsyncSelect<Accrediteds>
            ...
            value={selectedVal?.length ? { employ: { name: selectedVal } } : undefined}
            defaultOptions={accredited?.data}
            loadOptions={loadOptions}
            onChange={onChange}
            getOptionLabel={({ employ }) => employ.name}
            getOptionValue={({ employ }) => employ.name}
