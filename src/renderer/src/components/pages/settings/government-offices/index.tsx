...
import GovernmentOfficesTable from './government-offices-table'
...
export default function GovernmentOfficesIndex() {
  ...
  const { data: governmentOfficeTable, refetch } = useQuery({
    queryKey: ['governmentOfficeTable'],
    queryFn: () =>
      getApi<Info[]>('/government-office', {
        headers: {
  ...
  const { mutate } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['addGovernmentOfficeForm'] })
      refetch()
      form.reset()
    },
    onError: (error: any) => {
