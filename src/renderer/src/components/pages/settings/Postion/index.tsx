...
import PostionTable from './PostionTable'
...
export default function PostionIndex() {
  ...
  const { data: PotionsRespTable, refetch } = useQuery({
    queryKey: ['PotionsRespTable'],
    queryFn: () =>
      getApi<Info[]>('/position', {
        headers: {
  ...
  const { mutate } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['addPotionsForm'] })
      refetch()
      form.reset()
    },
    onError: (error: any) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <PostionTable info={PotionsRespTable?.data || []} pageSize="5" page="2" total={5} />
    </div>
  )
}
