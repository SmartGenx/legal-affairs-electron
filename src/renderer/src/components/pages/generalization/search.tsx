...
const GeneralizationSearch = () => {
  ...
  const loadOptions = async (value: string) => {
    if (!value) return []
    const data = await getApi<data[]>('/generalization', {
      params: {
  ...
  const onChange = (val: { title: string } | null) => {
    const params = new URLSearchParams(searchParams.toString())
    if (val?.title) {
      params.set('query', val.title)
    } else {
      params.delete('query')
    }
    params.set('page', '1')
    ...
    <div>
      <div className="!my-10 flex flex-col items-center justify-between sm:flex-row">
        <div className="flex w-full items-center justify-center rounded-xl border-[3px] border-[#E5E7EB] ">
          ...
          <AsyncSelect<data>
            placeholder="ابحث بموضوع التعميم.."
            loadingMessage={() => 'جارٍ البحث ...'}
            noOptionsMessage={() => 'لا توجد نتائج'}
            cacheOptions
            instanceId="products-search"
            value={selectedVal?.length ? { title: selectedVal } : undefined}
...
export default GeneralizationSearch
