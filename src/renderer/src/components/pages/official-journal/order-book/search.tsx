...
const OrderBookSearch = () => {
  ...
  const loadOptions = async (value: string) => {
    if (!value) return []
    const data = await getApi<Accrediteds[]>('/book-order', {
      params: {
        'include[Book]': true,
  ...
  const onChange = (val: Accrediteds | null) => {
    ...
    if (val?.Book.name) {
      params.set('query', val.Book.name)
    } else {
      params.delete('query')
    }
    params.set('page', '1')
      ...
      <div className="!my-10 flex flex-col items-center justify-between sm:flex-row">
        <div className="flex w-full items-center justify-center rounded-xl border-[3px] border-[#E5E7EB] ">
          ...
          <AsyncSelect<Accrediteds>
            ...
            value={selectedVal?.length ? { Book: { name: selectedVal } } : undefined}
            defaultOptions={accredited?.data}
            loadOptions={loadOptions}
            onChange={onChange}
            getOptionLabel={({ Book }) => Book.name}
            getOptionValue={({ Book }) => Book.name}
