...
const AgencyPdf = React.forwardRef<HTMLDivElement, Props>(function ComponentToPrint({ data }, ref) {
    ...
    <div ref={ref} className="w-[95%] mx-auto h-full relative">
      ...
      <table dir="rtl" className="w-full h-full rounded-t-lg">
        <tr className="  h-10 text-black border-2 border-gray-300 py-5 bg-[#eef0f8]">
          ...
          <th className="translate-x-4 font-arial">الاسم القانوني</th>
          <th className="translate-x-9 font-arial">المرفق الذي يمثله</th>
          <th className="translate-x-7 font-arial">المرفق الحكومي</th>
          <th className="translate-x-7 font-arial">تاريخ الإضافة</th>
        </tr>
        {data.map((item, index) => (
          // eslint-disable-next-line react/jsx-key
          <tr className=" -translate-x-3 h-10 text-black border-2 border-gray-300 py-5">
            <td className="px-2 font-arial">{index + 1}</td>
            <td className="font-arial">{item.legalName}</td>
            <td className="font-arial">{item.providedDocument}</td>
