...
export default function EditRoles({ id }: Props) {
  ...
  const { data: RoleByIdData, isSuccess } = useQuery({
    ...
    queryFn: async () =>
      await getApi<RoleById>(`/Role/${id}`, {
        headers: {
          Authorization: authToken()
        }
      })
  })
console.log("RoleByIdData",RoleByIdData?.data.name)
  // 1. Define your form.
  ...
  useEffect(() => {
    if (isSuccess) {
      form.reset({
        name: RoleByIdData.data.name
      })
    }
  }, [RoleByIdData])
  const { mutate } = useMutation({
    mutationKey: ['editRoleById'],
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="col-span-2">
                  <FormControl>
                    <Input label="اسم الدور" placeholder="اسم الدور" type="text" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
