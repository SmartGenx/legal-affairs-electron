...
const DecisionsSearch = () => {
  ...
  const loadOptions = async (value: string) => {
    if (!value) return []
    const data = await getApi<data[]>('/decision', {
      params: {
        'nameSource[contains]': value
      },
      headers: {
        Authorization: authToken()
      }
  ...
  const onChange = (val: { nameSource: string } | null) => {
    const params = new URLSearchParams(searchParams.toString())
    if (val?.nameSource) {
      params.set('query', val.nameSource)
    } else {
      params.delete('query')
    }
    params.set('page', '1')
      ...
      <div className="!my-10 flex flex-col items-center justify-between sm:flex-row">
        <div className="flex w-full items-center justify-center rounded-xl border-[3px] border-[#E5E7EB] ">
          ...
          <AsyncSelect<data>
            ...
            value={selectedVal?.length ? { nameSource: selectedVal } : undefined}
            defaultOptions={issuesSearch?.data}
            loadOptions={loadOptions}
            onChange={onChange}
            getOptionLabel={({ nameSource }) => nameSource}
            getOptionValue={({ nameSource }) => nameSource}
