    ...
    <div ref={ref} className="w-[95%] mx-auto h-full relative">
      ...
      <table dir="rtl" className="w-full h-full rounded-t-lg">
        <tr className="  h-10 text-black border-2 border-gray-300 py-5 bg-[#eef0f8]">
          ...
          <th className='font-arial'>رقم القرار</th>
          <th className="translate-x-4 font-arial">تاريخ القرار</th>
          <th className="translate-x-4 font-arial">جهة القرار</th>
          <th className="translate-x-7 font-arial">اسم صاحب القرار</th>
          <th className="translate-x-4 font-arial">تاريخ الإضافة</th>
        </tr>
        {data.map((item, index) => (
          ...
          <tr className=" -translate-x-3 h-10 text-black border-2 border-gray-300 py-5">
            ...
            <td className='font-arial'>{new Date(item.decisionDate).toISOString().split('T')[0]}</td>
            <td className='font-arial'>{item.governmentOffice.name}</td>
            <td className='font-arial'>{item.nameSource}</td>
