...
class ComplaintService {
    ...
    async createComplaint(complaintData) {
        try {
            ...
            const existingComplaint = await prisma.complaint.findFirst({ where: { name : name, governmentOfficeId: governmentOfficeId } });
    ...
    async updateComplaint(id, complaintData) {
        try {
            ...
            if (name) {
                const existingComplaint = await prisma.complaint.findFirst({ where: { name, governmentOfficeId: governmentOfficeId } });
                if (existingComplaint) {
                    throw new NotFoundError(`Complaint with name ${name}  already exists.`);
                }
            }
            if(governmentOfficeId){
                const existingGovernmentOffice = await prisma.governmentOffice.findUnique({ where: { id: governmentOfficeId,name:name } });
                if (existingGovernmentOffice) {
                    throw new NotFoundError(`Government Office with ID ${governmentOfficeId}  with name ${name} is exist.`);
                }
